...
void OMMenuMgr::setKeypadInput(Keypad* keypad, MenuKeyMap* keyMap) {


    if( m_inputType != MENU_KEYPAD )
        return;
...
int OMMenuMgr::_checkKeypad() {
    ...
    for (int i=0;i<5;i++){
       if (keyPressed == m_menuKeys->character[i]){
         return m_menuKeys->action[i];
       }
     }
